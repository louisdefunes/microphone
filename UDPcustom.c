...
/* Efektem jest stworzenie socketa UDP oraz przypisanie mu
...
int initialize_udp(struct sockaddr_in *client_address,
    ...
    /* Wypelnianie struktur adresów klienta(esp32) i serwera odbierającego
       server_address jest wypełniany i używany poza funkcją, client_address
       używany do przypisania socketowi */
...
{
    //rzutowanie pvParameters na strukturę zdefiniowaną w pliku task_structs.h
    udp_send_config_t *udp_conf = (udp_send_config_t *)pvParameters;

    //inicjalizacja struktur adresowych serwera i kleinta
